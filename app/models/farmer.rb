class Farmer
  ...
  @last_seeded = 100 # Today's date will never match this, forcing a seed on the first load

  def self.seed_if_needed
    # The idea is to seed every night at midnight, as a way of flushing the system so someone else will see the freshly seeded rumors
    # To seed lazily, we just seed if it hasn't been seeded yet "today"
    unless @last_seeded == self.today
      self.seed
      self.set_last_seeded
      puts 'seeded'
    end
  end
  ...
  def self.set_last_seeded
    @last_seeded = self.today
  end
